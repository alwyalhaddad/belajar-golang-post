...
func UpdateCartItemQuantity(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err = db.Preload("Cart").First(&cartItem, cartItemID).Error
		...
		// Verification cart to authenticated user
		if cartItem.Cart.UserID != int64(userID) {
			responses.Error(c, http.StatusUnauthorized, "Unauthorized", "Cart is not yours")
			return
		}
		...
		if request.Quantity == 0 {
			...
			responses.Success(c, http.StatusNoContent, "Item Remove Sucessfully", nil)
			return
		}
...
func RemoveCartItem(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Verfication cart to authenticated user
		if cartItem.Cart.UserID != int64(userID) {
			responses.Error(c, http.StatusUnauthorized, "Unauthorized", "This cart is not yours")
		}
